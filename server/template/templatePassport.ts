...
export const templatePassport = (data: Doc) => {
    ...
    const tempOthers = otherFiles
        ? otherFiles
              ?.map((file) => imgStr({ img: file, date: dateFrmat, noDocument, name, allPages, currentDopPages: (currentDopPages += 1) }))
              .join("")
        : "";
    ...
    const { head, str1, str2, str3, str4, str5, str6, str7, str8, str9, str10, str11 } = staticStr({
        name,
        factoryNumber,
        otherFiles,
        noDocument,
