...
const FormCard: FC<FormCardProps> = (props) => {
    ...
    const { name, factoryNumber, length, heigth, width, filePrimary, fileSecondary, fileSpec, otherFiles } = useSelector(getFormData)
    const { noDocument, noOrder, date, miniDoc } = useSelector(getSetingDocumentData)
    const cards = useSelector(getCards)
    const valid = useFormValid([name, factoryNumber, length, heigth, width, filePrimary, fileSecondary])
    ...
    const onCreateDocHandler = useCallback(async () => {
        if (
            ...
            const data: CardSchema = {
                ...
                name: name,
                factoryNumber: `АН-${factoryNumber}-${dayjs(date).format("YYYY")}-${noOrder}-${String(noDocument).slice(1)}`,
                length: length,
                width: width,
                heigth: heigth,
                date: date,
    ...
    }, [dispatch, factoryNumber, filePrimary, fileSecondary, fileSpec, heigth, length, name, width, date, noDocument, noOrder, otherFiles, cards])
    ...
    const onfactoryNumberChange = useCallback((value: string) => {
        dispatch(formCardActions.setFactoryNumber(value))
    }, [dispatch, factoryNumber])

    const onLengthChange = useCallback((value: string) => {
    ...
    const onHeigthChange = useCallback((value: string) => {
        const valueNum = Number(value.trim())
        if (!isNaN(valueNum) && valueNum < MAX_VALUE_NUM) {
    ...
    const onWidthChange = useCallback((value: string) => {
        const valueNum = Number(value.trim())
        if (!isNaN(valueNum) && valueNum < MAX_VALUE_NUM) {
    ...
    const onFilePrimaryChange = useCallback(async (files: FileList | null) => {
        if (files?.length) {
    ...
    const onFileSecondaryChanges = useCallback(async (files: FileList | null) => {
        if (files?.length) {
            const result = await encoding(files[0])
    ...
    const onFileSpecChanges = useCallback(async (files: FileList | null) => {
        if (files?.length) {
            const result = await encoding(files[0])
            dispatch(formCardActions.setFileSpec(result))
        } else {
            dispatch(formCardActions.setFileSpec(null))
        }
    }, [dispatch, fileSpec])

    const onOtherFilesChanges = useCallback(async (files: FileList | null) => {
    ...
    return <form className={`${cls.formCard} ${classNames}`}>
        <Button classNames={cls.exit} onClick={onExitHandler}>&#10006;</Button>
        <div className={cls.inputWrapper}>
            ...
            <Input value={heigth} onChange={onHeigthChange} placeholder="Высота изделия" />
        </div>
        <div className={cls.inputWrapper}>
            ...
            <InputFile file={fileSpec} onChange={onFileSpecChanges} placeholder="Спецификация" />
            <InputFile file={otherFiles} onChange={onOtherFilesChanges} placeholder="Доп фото" multiple={true} />
        </div>
        <div className={cls.buttonWrapper}>
            <Button classNames={cls.button} onClick={onCreateDocHandler} disabled={!valid}>Добавить паспорт</Button>
        </div>
    </form>
}
